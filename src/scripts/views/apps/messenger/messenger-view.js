...
import HeaderBarView from '../../../views/apps/messenger/header-bar/header-bar-view.js';
import SideBarView from '../../../views/apps/messenger/sidebar/sidebar-view.js';
import TabbedContentView from '../../../views/apps/messenger/mainbar/tabbed-content/tabbed-content-view.js';
import FooterBarView from '../../../views/apps/messenger/footer-bar/footer-bar-view.js';
...
export default AppSplitView.extend(_.extend({}, MultiDoc, ContainableSelectable, MultiSelectable, Openable, LinkShareable, ItemInfoShowable, TopicInfoShowable, ChatInfoShowable, {
	...
	shareTopic: function(topic) {
		import(
			'../../../views/apps/messenger/dialogs/invitations/share-topic-by-invitation-dialog-view.js'
	...
	*/
	...
	/*
	openChat: function(which) {
		let listView = this.getChildView('sidebar').getChildView('chats').getChildView('items').getChildView('items');
		let next;

		if (listView.isEmpty()) {
			this.showChatInvitationsDialog();
			return;
		}

		switch (which) {
			case 'first':
				next = listView.getChildView('first');
				break;
			case 'prev':
				next = listView.getChildView('prev');
				break;
			case 'next':
				next = listView.getChildView('next');
				break;
			case 'last':
				next = listView.getChildView('last');
				break;
		}
	...
	*/
	...
	showPreferencesDialog: function() {
		import(
			'../../../views/apps/messenger/dialogs/preferences/preferences-dialog-view.js'
