...
class VerifyStorageAccess
{
	...
	public function setLinkContext(Request $request, $linkId) {
		...
		if (str_starts_with($link->path, '/')) {
			UserStorage::push($link->path);
			return;
		}

		// find link's home directory
		//
		if (!$userAccount->isAdmin()) {
			$homedir = $userAccount->username;
		} else {
			$homedir = '';
		}
		...
		//
		UserStorage::push($homedir . '/' . $link->path);
	}
	...
	public function setFileContext(Request $request, $fileId) {
		...
		// admin users use root directory as home
		//
		if ($userAccount->user->isAdmin()) {
			$username = '';
		}
