...
export default File.extend({
		...
		switch (attributeName) {
			...
			case 'track_number':
				...
				if (trackNumber && trackNumber.contains('/')) {
					trackNumber = parseInt(trackNumber.split('/')[0]);
				} else {
					trackNumber = parseInt(trackNumber);
				}
				return trackNumber;
			case 'length':
				return this.has('id3')? parseInt(this.get('id3').length[0]) : undefined;
			default:
	...
	showInfo: function(options) {
		import(
			...
			//
			application.show(new AudioFileInfoDialogView.default(_.extend({
				model: this
			}, options)));
		});
	}
